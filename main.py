...
end_time_formatted = datetime.fromtimestamp(END_TIME)  # Convert float to datetime
...
video_data = {
    ...
    # "END_TIME": END_TIME.strftime("%d/%m/%Y %H:%M:%S") if VID_FPS > 0 else "Unknown"
    "END_TIME": end_time_formatted
}
print(video_data)

def datetime_handler(x):
    if type(x) == datetime.datetime:  # Using type check
        return x.isoformat()
