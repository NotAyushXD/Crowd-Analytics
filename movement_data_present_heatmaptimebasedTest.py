...
def draw_heatmap_scores(frame, heatmap_scores, grid_size):
    height, width = frame.shape[:2]
    cell_height = height // grid_size
    cell_width = width // grid_size
    ...
    for i in range(grid_size):
        for j in range(grid_size):
            score = heatmap_scores[i * grid_size + j]
            position = (j * cell_width + 10, i * cell_height + 30)  # Adjust position as needed
            cv2.putText(frame, str(score), position, cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
...
with open('./processed_data/heatmap_scores.csv', 'w', newline='') as csvfile:
    ...
    for _ in tqdm(range(total_frames)):
        ...
        # Draw the grid on the heatmap frame
        draw_grid(heatmap_frame, grid_size)

        # Aggregate the heatmap over aggregation_interval frames
        if aggregated_heatmap is None:
            aggregated_heatmap = np.zeros_like(heatmap, dtype=np.float32)
        aggregated_heatmap = cv2.addWeighted(aggregated_heatmap, 1.0, heatmap.astype(np.float32), 1.0, 0.0)

        interval_count += 1

        if interval_count == aggregation_interval:
            # Normalize aggregated heatmap
            aggregated_heatmap /= aggregation_interval

            # Draw heatmap scores on the frame
            heatmap_scores = calculate_grid_heatmap_scores(aggregated_heatmap, grid_size)
            draw_heatmap_scores(heatmap_frame, heatmap_scores, grid_size)

            # Save heatmap image
            heatmap_filename = os.path.join(heatmap_dir, f'heatmap_frame_{frame_count}.jpg')
            cv2.imwrite(heatmap_filename, heatmap_frame)

            # Write heatmap scores to CSV
            csv_writer.writerow([timestamp] + heatmap_scores)

            # Reset aggregated heatmap and interval count
            aggregated_heatmap = None
            interval_count = 0

        # Display or save the frame with heatmap
        cv2.imshow('Heatmap Frame', heatmap_frame)
        cv2.waitKey(1)
